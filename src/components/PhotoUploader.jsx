...
const PhotoUploader = ({addedphotos,onChange}) => {
    ...
    const [imageUrls, setImageUrls] = useState('');
     ...
     const addByLink = async (e) => {
       ...
       try{
            ...
            console.log(CLOUDINARY_URL+'/'+data.name);
            onChange( prev => {
    ...
    const uploadPictureFile = async (e) => {
       const file = e.target.files[0];
        ...
        try {
            const response = await axios.post('/upload/by-file', data, {
           ...
           console.log("recived this ",url);
        } catch (error) {
            console.error('Error uploading image:', error.response ? error.response.data.error : error.message);
        }
    };
                                ...
                                addedphotos.length > 0 && addedphotos.map((link,index) => {
                                    return (  
                                        <div key={link} className="h-32 flex relative"  onClick={(e) => makeMainPicture(e,link)}>
                                            ...
                                            <img className="rounded-2xl w-full object-cover " src={`${CLOUDINARY_URL}/${link}`} alt={link} />
